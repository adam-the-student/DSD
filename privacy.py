...
# =====================================================================
# CRYPTOGRAPHIC INITIALIZATION LAYER
# =====================================================================
try:
...
def save_anonymized_and_encrypted_frame(frame_or_crop, pose_results, prefix="sample", extra_suffix="", crop_offsets=None, is_rad_tech=False):
    """
    Saves an encrypted chest crop. 
    Only blurs if a significant upper face area is visible, and completely skips blurring if the person is a Rad Tech.
    """
    processed_img = frame_or_crop.copy()
    h, w, _ = processed_img.shape
    offset_x, offset_y = crop_offsets if crop_offsets else (0, 0)

    # 🟢 RULE 1: If the Stage 3 classifier identifies them as a Rad Tech, SKIP BLURRING COMPLETELY
    if is_rad_tech:
        # We still print a console note so you can verify it in your SSH terminal log
        pass 
            ...
            if result.keypoints is not None and len(result.keypoints.xy) > 0:
                ...
                for idx, kp in enumerate(keypoints[:5]):
                    if kp[0] > 0 and kp[1] > 0:
                        ...
                        # Verify if this specific keypoint actually lands inside the saved crop canvas
                        if 0 <= rel_x <= w and 0 <= rel_y <= h:
                            face_pts_in_crop.append([rel_x, rel_y])
                            if idx in [0, 1, 2]:  # Track upper features (Nose and Eyes)
                                upper_face_count += 1
                
                # 🟢 RULE 2: Only blur if a substantial portion of the upper face is leaking into the crop
